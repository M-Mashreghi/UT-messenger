...
inline int phonenumberchecker(char* phonenumber) {
...
inline int passwordchecker(char* pass) {
	if (strlen(pass) < 6) {
...
inline void adduser(user* head, user* neww)
{
	user* now = head->next;
	user* pre = head;
	int i = zero;
...
inline void passchange(user* neww) {
...
inline void changeusername(user* head, user* neww) {
...
inline int checkreciverexists(user* head, char* reciver) {
	struct user* current = head->next;
	while (current != NULL)
	{
...
inline void addmessage(userMessage* messagehead, userMessage* message) {
...
inline int findusernameforlogin1(char* Username, char* Password, user* head) {
	struct user* current = head->next;
	while (current != NULL)
	{
...
inline void showsentmssage(user* neww, userMessage* messagehead) {
	char c;
	int z = zero, i = zero;
...
inline void showreceivedmessage(user* neww, userMessage* messagehead) {
...
inline struct user* findusernameforlogin(char* Username, user* head) {
...
inline int usernameCheckforsignup(user* head, char* NewUser)
{
	user* curr = head->next;
	while (curr != NULL)
	{
...
inline void loginmenu(user* head, user* neww, userMessage* messagehead) {
	char c; int i, a, flag;
	while (one) {
...
inline int PhoneNumCheckforsignup(user* head, char* phone)
{
	user* curr = head->next;
	while (curr != NULL)
	{
...
inline void savemessage(userMessage* messagehead) {
	FILE* message;
...
inline void freerammessage(userMessage* messagehead)
{
	struct userMessage* curr = messagehead->next;
...
inline void freeramuser(user* head) {
...
inline void adduserr(char* Username, char* Password, char* PhoneNum, user* usersign) {
...
inline void showunreadmessage(user* neww, userMessage* messagehead) {
	struct userMessage* current = messagehead->next;
	while (current != NULL)
	{
...
inline void addmssage(char* contect, char* reciver, char* sender, userMessage* message) {
...
inline void saveuser(user* head) {
	FILE* users;
...
inline void sendmessage(user* head, user* neww, userMessage* messagehead) {
